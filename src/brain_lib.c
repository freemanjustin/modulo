...
void sync_update_npat( RUN_TIME_TYPE *p, NEURONE_TYPE *neurone ){
	...
	for ( cycles = 0; cycles < p->update_cycles; cycles++ ){
			...
			printf("++finished an update cycle \n");
			...
			*/
			...
			if( sorted_array[ (n - (p->actives-1)) ] == sorted_array[ (n - (p->actives)) ] ){
					...
					for(j=0; j<start-1; j++){
								//printf("tagging a module as large\tmodule = %d (j = %d, n = %d, (n-j) = %d) start = %d\n",(int)index[(n-j)],j,n,n-j,start);
								//fflush(stdout);
					...
					do{
								...
								random_val = (int)floor(((end-start)+1) * ran4());
								//printf("random_val = %d\n",random_val);
								if( (neurone+(int)index[(n-start-random_val)])->largest != 1){
										//printf("\tselected a module to tag, module = %d\n",(int)index[(n-start-random_val-1)]);
										(neurone+(int)index[(n-start-random_val)])->largest = 1;
										num_active++;
								}
